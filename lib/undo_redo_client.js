...
var undoneRedoConditions = function () {
  ...
  var lastAction = tx.Transactions.findOne({user_id: meteorUserId(), $or: [{undone: null}, {undone: {$exists: false}}], expired: {$exists: false}}, {sort: {lastModified: -1}});
  if (lastAction) {
...
Template.undoRedoButtons.helpers({
  ...
  hideRedoButton : function() {
    return (tx.Transactions.find({user_id: meteorUserId(), undone: undoneRedoConditions(), expired: {$exists: false}}).count()) ? '' : 'hide-redo-button';
  },
